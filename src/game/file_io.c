...
int game_file_io_read_scenario_info(const char *filename, saved_game_info *info)
{
    ...
    if (version > SCENARIO_LAST_STATIC_ORIGINAL_DATA) {
        ...
        info->total_invasions = scenario_invasions_from_buffer(state->invasions, version);
    }
    ...
    savegame_version_t version, memory_block *compress_buffer)
{
    ...
    if (version_data.features.dynamic_scenario_objects) {
        skip_piece(fp, PIECE_SIZE_DYNAMIC, 1);
        skip_piece(fp, PIECE_SIZE_DYNAMIC, 1);
        skip_piece(fp, PIECE_SIZE_DYNAMIC, 1);
        skip_piece(fp, PIECE_SIZE_DYNAMIC, 1);
        skip_piece(fp, PIECE_SIZE_DYNAMIC, 1);
    ...
    skip_piece(fp, 4, 0);
    skip_piece(fp, 12, 0);
    skip_piece(fp, version_data.piece_sizes.invasion_warnings, 1);
    ...
    scenario_description_from_buffer(&scenario_piece.buf, info->description, scenario_version);
    info->image_id = scenario_image_id_from_buffer(&scenario_piece.buf, scenario_version);
    info->climate = scenario_climate_from_buffer(&scenario_piece.buf, scenario_version);
    info->total_invasions = scenario_invasions_from_buffer(&scenario_piece.buf, scenario_version);
    info->player_rank = scenario_rank_from_buffer(&scenario_piece.buf, scenario_version);
    info->start_year = scenario_start_year_from_buffer(&scenario_piece.buf, scenario_version);
    scenario_open_play_info_from_buffer(&scenario_piece.buf, scenario_version, &info->is_open_play, &info->open_play_id);

    if (!info->is_open_play) {
        scenario_objectives_from_buffer(&scenario_piece.buf, scenario_version, &info->win_criteria);
    }
