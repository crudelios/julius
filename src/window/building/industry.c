...
void window_building_industry_get_tooltip(building_info_context *c, int *group_id, int *text_id)
{
    ...
    if (type == BUILDING_CITY_MINT) {
        ...
        y_offset = needed_resources * 20;
    } else {
        y_offset = 16;
    }

    if (window_building_get_risks_tooltip(c, c->x_offset + c->width_blocks * BLOCK_SIZE - 76,
            c->y_offset + 154 + y_offset, group_id, text_id)) {
        return;
    }

    // Shipyards and wharves don't display efficiency info
    ...
    if (m->x >= c->x_offset + 16 && m->x < c->width_blocks * BLOCK_SIZE + c->x_offset - 16 &&
        m->y >= c->y_offset + 60 + y_offset && m->y < c->y_offset + 86 + y_offset) {
        *group_id = CUSTOM_TRANSLATION;
        *text_id = TR_BUILDING_WINDOW_INDUSTRY_EFFICIENCY_TOOLTIP;
    }
}
